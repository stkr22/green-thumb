...
- `db.py` sets WAL + foreign_keys + busy_timeout pragmas per connection. The backend must stay single-instance: the daily reminder loop runs in-process, so multiple replicas would double-send notifications.
